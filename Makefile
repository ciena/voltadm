help:
	...
	@echo "run          - runs voltadm using the command specified as \$$CMD"
...
LDFLAGS=-ldflags \
	"-X \"github.com/opencord/voltadm/internal/pkg/cli/version.Version=$(VERSION)\"  \
	 -X \"github.com/opencord/voltadm/internal/pkg/cli/version.VcsRef=$(GITCOMMIT)\"  \
	 -X \"github.com/opencord/voltadm/internal/pkg/cli/version.VcsDirty=$(GITDIRTY)\"  \
	 -X \"github.com/opencord/voltadm/internal/pkg/cli/version.GoVersion=$(GOVERSION)\"  \
	 -X \"github.com/opencord/voltadm/internal/pkg/cli/version.Os=$(HOST_OS)\" \
	 -X \"github.com/opencord/voltadm/internal/pkg/cli/version.Arch=$(HOST_ARCH)\" \
	 -X \"github.com/opencord/voltadm/internal/pkg/cli/version.BuildTime=$(BUILDTIME)\""
...
release:
	...
	@${GO_SH} ' \
	  ...
	  for x in ${RELEASE_OS_ARCH}; do \
	    ...
	    echo "$$OUT_PATH"; \
	    GOOS=$${x%-*} GOARCH=$${x#*-} go build -mod=vendor -v $(LDFLAGS) -o "$$OUT_PATH" cmd/voltadm/voltadm.go; \
	  done'
...
build: local-lib-go
	go build -mod=vendor $(LDFLAGS) cmd/voltadm/voltadm.go

install:
	go install -mod=vendor $(LDFLAGS) cmd/voltadm/voltadm.go

run:
	go run -mod=vendor $(LDFLAGS) cmd/voltadm/voltadm.go $(CMD)
...
clean:
	rm -rf voltadm voltadm.cp release sca-report
